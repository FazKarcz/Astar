...
vector<pair<int, int>> znajdzSciezke(const vector<vector<int>>& mapa, const pair<int, int>& start, const pair<int, int>& meta) {
    ...
    while (!zbiorOtwarty.empty()) {
        ...
        if (obecny->x == wezelMeta->x && obecny->y == wezelMeta->y) {
            ...
            cout << "Ile krokow zrobilem: " << kroki << endl;
            return sciezka;
        }
...
void rysujMape(const vector<vector<int>>& mapa, const vector<pair<int, int>>& sciezka, const pair<int, int>& start, const pair<int, int>& meta) {
    for (int i = 0; i < mapa.size(); ++i) {
        for (int j = 0; j < mapa[i].size(); ++j) {
            bool jestSciezka = false;
            for (const auto& punkt : sciezka) {
                if (punkt.first == i && punkt.second == j) {
                    jestSciezka = true;
                    break;
                }
            }
...
int main() {
    ...
    rysujMape(mapa, sciezka, start, meta);
